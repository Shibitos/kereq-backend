CREATE TABLE USERS (
	USER_ID BIGINT PRIMARY KEY,
	USER_AUDIT_CD TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
	USER_AUDIT_CU BIGINT,
	USER_AUDIT_MD TIMESTAMP,
	USER_AUDIT_MU BIGINT,
	USER_AUDIT_RD TIMESTAMP,
	USER_VERSION BIGINT NOT NULL DEFAULT 0,
	USER_PASSWORD VARCHAR(72) NOT NULL,
	USER_EMAIL VARCHAR(50) NOT NULL,
	USER_FIRST_NAME VARCHAR(35) NOT NULL,
	USER_LAST_NAME VARCHAR(40) NOT NULL,
	USER_BIOGRAPHY VARCHAR(200),
	USER_BIRTH_DATE TIMESTAMP NOT NULL,
	USER_COUNTRY VARCHAR(30) NOT NULL,
	USER_GENDER VARCHAR(1) NOT NULL,
	USER_ACTIVATED BOOLEAN NOT NULL DEFAULT FALSE
);

CREATE SEQUENCE SEQ_USER_ID START 50 INCREMENT 50;
CREATE UNIQUE INDEX IDX_UQ_USER_EMAIL ON USERS(UPPER(USER_EMAIL));

CREATE TABLE ROLES (
    ROLE_ID BIGINT PRIMARY KEY,
	ROLE_AUDIT_CD TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
	ROLE_AUDIT_CU BIGINT,
	ROLE_AUDIT_MD TIMESTAMP,
	ROLE_AUDIT_MU BIGINT,
	ROLE_AUDIT_RD TIMESTAMP,
	ROLE_VERSION BIGINT NOT NULL DEFAULT 0,
	ROLE_CODE VARCHAR(20) UNIQUE NOT NULL,
	ROLE_NAME VARCHAR(25) NOT NULL
);

CREATE SEQUENCE SEQ_ROLE_ID START 50 INCREMENT 50;

CREATE TABLE USERS_ROLES (
    UR_USER_ID BIGINT REFERENCES USERS(USER_ID),
    UR_ROLE_ID BIGINT REFERENCES ROLES(ROLE_ID),
    PRIMARY KEY (UR_USER_ID, UR_ROLE_ID)
);

INSERT INTO ROLES (ROLE_ID, ROLE_CODE, ROLE_NAME)
VALUES
    (NEXTVAL('SEQ_ROLE_ID'), 'ROLE_USER', 'User'),
    (NEXTVAL('SEQ_ROLE_ID'), 'ROLE_ADMIN', 'Administrator');
